...
/**
 * Native-image compatible version of ApplicationMainSingleWindow.
 * Can be run with image directory as first argument or uses Pictures folder.
 *
 * Usage: ./CanvasImageGrid [image-directory]
 */
public class ApplicationMainNative extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        // Get image directory from arguments or use Pictures folder
        Path rootFolder;
        List<String> params = getParameters().getRaw();

        if (params != null && !params.isEmpty()) {
            rootFolder = Paths.get(params.get(0));
            System.out.println("Loading images from: " + rootFolder);
        } else {
            // Use Pictures directory or current directory
            String homeDir = System.getProperty("user.home");
            rootFolder = Paths.get(homeDir, "Pictures");

            if (!Files.exists(rootFolder)) {
                rootFolder = Paths.get(".");
            }
            System.out.println("No image directory specified, using: " + rootFolder);
            System.out.println("Usage: ./CanvasImageGrid <image-directory>");
        }

        List<Path> imagePaths = new ArrayList<>();

        if (Files.exists(rootFolder) && Files.isDirectory(rootFolder)) {
            imagePaths = getImageFiles(rootFolder);
        }
        ...
        stage.setTitle("CanvasImageGrid - Native");
        ...
        if (!imagePaths.isEmpty()) {
            ...
            System.out.println("Loaded " + imagePaths.size() + " images");
        } else {
    ...
    private List<Path> getImageFiles(Path root) {
        final List<Path> imagePaths = new ArrayList<>();
